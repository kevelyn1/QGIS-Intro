...
### 1. Installation
**These tutorials are made with QGIS version 3.28 for Windows** and therefore it is the easiest to follow them in version 3.28 and on a PC. However, you can also easily follow them when using the latest stable version QGIS 3.40. QGIS can be downloaded from [here](https://qgis.org/en/site/forusers/download.html). Other QGIS versions are not recommended because they are not stable. Version 3.28 can be downloaded from [here](https://download.qgis.org/downloads/)
